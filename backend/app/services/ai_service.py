"""Anthropic-backed AI assistive service for ConceptPilot.

Features:
  - Concept tag suggestion from question text
  - Prerequisite edge suggestion from concept list
  - Subtopic expansion for knowledge graph
...
logger = logging.getLogger("conceptpilot.ai")
...
async def _call_anthropic(
    ...
    """Make a structured JSON call to Anthropic with error handling.
    ...
    try:
        ...
        result["latency_ms"] = round((time.time() - start) * 1000, 2)
        logger.error("Anthropic JSON parse error", extra={"request_id": request_id, "error": str(e)})

    except anthropic.APITimeoutError as e:
        result["error"] = f"Anthropic timeout error: {str(e)}"
        result["latency_ms"] = round((time.time() - start) * 1000, 2)
        logger.error("Anthropic timeout error", extra={"request_id": request_id, "error": str(e)})

    except anthropic.APIConnectionError as e:
        result["error"] = f"Anthropic connection error: {str(e)}"
        result["latency_ms"] = round((time.time() - start) * 1000, 2)
        logger.error("Anthropic connection error", extra={"request_id": request_id, "error": str(e)})

    except anthropic.RateLimitError as e:
        result["error"] = f"Anthropic rate limit: {str(e)}"
        result["latency_ms"] = round((time.time() - start) * 1000, 2)
        logger.warning("Anthropic rate limited", extra={"request_id": request_id})

    except Exception as e:
        result["error"] = f"Anthropic unexpected error: {str(e)}"
        result["latency_ms"] = round((time.time() - start) * 1000, 2)
        logger.exception("Anthropic unexpected error", extra={"request_id": request_id})
...
async def suggest_concept_tags(
    ...
    """Suggest concept tags for a question using Anthropic."""
...
Analyze this question and suggest which concepts it tests."""
    ...
    result = await _call_anthropic(CONCEPT_TAG_SYSTEM, user_prompt)
...
async def suggest_prerequisite_edges(
    ...
    """Suggest prerequisite edges between concepts using Anthropic."""
...
Suggest prerequisite relationships between these concepts. A prerequisite means the source concept should be learned before the target concept."""
    ...
    result = await _call_anthropic(PREREQ_EDGE_SYSTEM, user_prompt)
...
async def suggest_subtopic_expansion(
    ...
    """Generate subtopic expansion for a concept node using Anthropic."""
...
Generate a knowledge graph expansion from "{concept_id}" going up to {max_depth} layers deep. Include the parent concept as an edge source where appropriate."""
    ...
    result = await _call_anthropic(SUBTOPIC_EXPAND_SYSTEM, user_prompt)
...
Generate targeted intervention recommendations for the weak concepts in this student cluster."""
    ...
    result = await _call_anthropic(INTERVENTION_SYSTEM, user_prompt)
