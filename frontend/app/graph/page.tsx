...
const ReactFlowCanvas = dynamic(
  () => import('@xyflow/react').then((m) => {
    ...
    return {
      default: ({ nodes, edges, onNodeClick, onEdgeClick, defaultEdgeOptions, children }: {
        ...
        <m.ReactFlow
          ...
          fitView defaultEdgeOptions={defaultEdgeOptions}
        >
          <m.Background gap={20} size={1} color={themeColor.border} />
          <m.Controls />
          <m.Panel position="bottom-left">
            {children}
          </m.Panel>
        </m.ReactFlow>
      ),
