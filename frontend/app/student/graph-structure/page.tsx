...
/** React Flow structure editor; `/student/graph` is the D3 knowledge graph view. */
export default function StudentGraphStructureEditorPage() {
  ...
  const handleAddNode = async () => {
    if (!newNodeLabel.trim() || !examId) return;
    try {
  ...
  const handleDeleteNode = async () => {
    if (!selectedNodeId || !examId) return;
  ...
  const handleNodeClick = useCallback(
    ...
    [connectingFrom, selectedNodeId, examId],
  );

  const handleDeleteEdge = async (edgeId: string) => {
    const edge = graphEdges.find((e) => e.id === edgeId);
    if (!edge || !examId) return;
    try {
  ...
  const validateReconnect = useCallback((oldEdge: Edge, newConnection: Connection) => {
    ...
    if (wouldReconnectCreateCycle(graphEdgesRef.current, oldEdge.source, oldEdge.target, ns, nt)) {
  ...
  const onReconnectPersist = useCallback(
    async (oldEdge: Edge, newConnection: Connection) => {
      ...
      await api.reconnectGraphEdge(examId, oldEdge.source, oldEdge.target, ns, nt);
      setGraphEdges((prev) => {
        ...
        <div className="relative px-6 py-4 border-b border-border bg-white/80 backdrop-blur-sm flex items-center justify-between flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Add/remove nodes and edges (React Flow). Knowledge Graph is a separate page for D3 + AI expand.
            </p>
            <Link
              href="/student/graph"
