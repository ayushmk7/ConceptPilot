...
import type { StudentConceptGraphNode } from '@/lib/graph-student-concepts';

interface StudentConceptGraphProps {
  concepts: StudentConceptGraphNode[];
...
interface SimLink extends d3.SimulationLinkDatum<SNode> {
  source: SNode | string;
  target: SNode | string;
}
...
export const StudentConceptGraph: React.FC<StudentConceptGraphProps> = ({
  ...
  useEffect(() => {
    ...
    simulation.on('tick', () => {
      linkSel
        .attr('x1', (d) => (d.source as SNode).x!)
        .attr('y1', (d) => (d.source as SNode).y!)
        .attr('x2', (d) => (d.target as SNode).x!)
        .attr('y2', (d) => (d.target as SNode).y!);
