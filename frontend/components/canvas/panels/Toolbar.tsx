        ...
        <div className="relative">
          ...
          {showAddMenu && (
            <>
              ...
              <div className="absolute top-full mt-2 left-0 z-50 bg-white rounded-lg shadow-lg border border-[#E2E8F0] py-2 min-w-[200px]">
                {nodeOptions.map((opt) => (
                  <button
                    key={opt.type}
                    onClick={() => {
                    ...
                    className="w-full text-left px-4 py-2.5 hover:bg-[#E8EEF4] transition-colors flex items-center gap-3"
                  >
                    <opt.icon className="w-4 h-4 text-[#4A5568]" />
                    <div>
                      <div className="text-sm text-[#1A1A2E]">{opt.label}</div>
